...
def fsdp_wrapper(original_model, fsdp_config, ignored_modules=[]):
    # original_model.language_model.to(torch.bfloat16) # 统一格式
    ...
    return FSDP(
        ...
        use_orig_params=True,#  if hasattr(original_model.language_model, "_peft_config") else False,
...
class FSDPCheckpoint:
    ...
    def fsdp_save_ckpt(
        ...
        save_path = os.path.join(ckpt_dir, f"{train_steps:07d}") if isinstance(train_steps, int) else os.path.join(ckpt_dir, f"{train_steps}")
        ...
        with FSDP.state_dict_type(
            ...
            if hasattr(model.language_model, "_peft_config"):
                model.language_model.save_pretrained(os.path.join(save_path, "lora_adapter"), is_main_process=(dist.get_rank() == 0))
            else:
                model_state_dict = model.state_dict()
                bf16_state_dict = {key: tensor.to(torch.bfloat16) for key, tensor in model_state_dict.items()}
                del model_state_dict
                if dist.get_rank() == 0:
                    save_file(bf16_state_dict, os.path.join(save_path, "model.safetensors"))
        if ema_model is not None:
            with FSDP.state_dict_type(
                ...
                if hasattr(ema_model.language_model, "_peft_config"):
                    ema_model.language_model.save_pretrained(os.path.join(save_path, "ema_lora_adapter"), is_main_process=(dist.get_rank() == 0))
                else:
                    ema_state_dict = ema_model.state_dict()
                    if dist.get_rank() == 0:
                        bf16_state_dict = {key: tensor.to(torch.bfloat16) for key, tensor in ema_state_dict.items()}
                        del ema_state_dict
                        if dist.get_rank() == 0:
                            save_file(bf16_state_dict, os.path.join(save_path, "ema.safetensors"))
    ...
    def try_load_ckpt(resume_from, logger, model, ema_model=None, resume_from_ema=False):
        if resume_from is not None and os.path.exists(resume_from):
            ...
            if resume_from_ema:
                if os.path.exists(os.path.join(resume_from, f"ema.safetensors")):
                    model_state_dict_path = os.path.join(resume_from, f"ema.safetensors")
                elif os.path.exists(os.path.join(resume_from, f"ema_bf16.safetensors")):
                    logger.info(f"Loading ema model in bf16 format from {resume_from}.")
                    model_state_dict_path = os.path.join(resume_from, f"ema_bf16.safetensors")
            else:
                if os.path.exists(os.path.join(resume_from, f"model.safetensors")):
                    model_state_dict_path = os.path.join(resume_from, f"model.safetensors")
            if model_state_dict_path is None:
                logger.info(f"No model checkpoint found in {resume_from}, training from scratch.")
            ...
            if ema_model is not None:
                if os.path.exists(os.path.join(resume_from, f"ema.safetensors")):
                    ...
                    logger.info(f"Loading ema model in bf16 format from {resume_from}.")
                    ema_state_dict_path = os.path.join(resume_from, f"ema_bf16.safetensors")
                else:
