...
class PackedAttention(Qwen2Attention):
    ...
    def forward_train(
        ...
        if isinstance(attention_mask, List):
            ...
            #     torch.cumsum(torch.tensor(sample_lens, device=packed_query_states.device), dim=0), 
            #     (1, 0)
            # ).to(torch.int32)
            
            # # 确保状态是 bfloat16
            # q = packed_query_states.to(torch.bfloat16)
            # k = packed_key_states.to(torch.bfloat16)
            # v = packed_value_states.to(torch.bfloat16)
            ...
            #     max_seqlen_k=max(sample_lens),
            #     causal=True, # 预训练通常是因果对齐
            # )
...
class PackedAttentionMoT(Qwen2Attention):
    ...
    def forward_train(
        ...
        if isinstance(attention_mask, List):
            ...
            ### 使用 flash attention 变长版本
            # device = packed_query_states_.device
            # actual_total_tokens = packed_query_states_.shape[0]
            # if sum(sample_lens) != actual_total_tokens:
            #     cu_seqlens = torch.tensor([0, actual_total_tokens], dtype=torch.int32, device=packed_query_states_.device)
            #     max_s = actual_total_tokens
            # else:
            #     # 正常匹配情况
            #     cu_seqlens = torch.nn.functional.pad(
            #         torch.cumsum(torch.tensor(sample_lens, device=packed_query_states_.device), dim=0), 
            #         (1, 0)
            #     ).to(torch.int32)
            #     max_s = max(sample_lens)
            
            # q = packed_query_states_.to(torch.bfloat16) 
            # k = packed_key_states_.to(torch.bfloat16)
            # v = packed_value_states.to(torch.bfloat16)

            # # 确保连续性以避免非法访问
            # q, k, v = q.contiguous(), k.contiguous(), v.contiguous()

            # assert cu_seqlens[-1] == q.shape[0], f"Total tokens mismatch: {cu_seqlens[-1]} vs {q.shape[0]}"
