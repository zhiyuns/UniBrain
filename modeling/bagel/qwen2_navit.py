...
class PackedAttentionMoT(Qwen2Attention):
    ...
    def forward(self, *args, **kwargs):
        use_cache = kwargs.pop('use_inference', False) or kwargs.get('use_cache', False) or (kwargs.get('past_key_values') is not None)
    ...
    def forward_train(
        ...
        if self.config.freeze_gen:
            packed_value_states[packed_gen_token_indexes] = packed_value_states[packed_gen_token_indexes].detach()
        ...
        if self.config.freeze_gen:
            packed_query_states_[packed_gen_token_indexes] = packed_query_states_[packed_gen_token_indexes].detach()
        ...
        if self.config.freeze_gen:
            packed_key_states_[packed_gen_token_indexes] = packed_key_states_[packed_gen_token_indexes].detach()
...
class Qwen2MoTDecoderLayer(nn.Module):
    def __init__(
        ...
        self.freeze_gen = config.freeze_gen
    ...
    def forward_train(
        ...
        if self.freeze_gen:
            if len(packed_gen_token_indexes) > 0:
                packed_sequence_[packed_gen_token_indexes] = packed_sequence_[packed_gen_token_indexes].detach()
...
class Qwen2MoEDecoderLayer(nn.Module):
    ...
    def forward(self, *args, **kwargs):
        use_cache = kwargs.get('use_cache', False) or (kwargs.get('past_key_values') is not None)
        if self.training and not use_cache:
            return self.forward_train(*args, **kwargs)
        else:
            return self.forward_inference(*args, **kwargs)
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def forward(self, *args, **kwargs):
        # DETECT SELF FORCING MODE:
        # If 'past_key_values' is provided, we must use the inference path 
        # to utilize the cache, even if we are in training mode.
        use_cache = kwargs.pop('use_inference', False) or kwargs.pop('use_cache', False) or (kwargs.get('past_key_values') is not None)

        if self.training and not use_cache:
            return self.forward_train(*args, **kwargs)
        else:
            # ARGUMENT MAPPING:
            # The caller (self_forcing_forward) might use training-style arg names.
            # We map them to inference-style arg names expected by forward_inference.
            
            # 1. packed_sequence -> packed_query_sequence
            if 'packed_sequence' in kwargs and 'packed_query_sequence' not in kwargs:
                kwargs['packed_query_sequence'] = kwargs.pop('packed_sequence')
            
            # 2. sample_lens (List[int]) -> query_lens (Tensor)
            if 'sample_lens' in kwargs and 'query_lens' not in kwargs:
                sample_lens = kwargs.pop('sample_lens')
                # forward_inference expects a Tensor for lengths
                device = kwargs['packed_query_sequence'].device
                kwargs['query_lens'] = torch.tensor(sample_lens, dtype=torch.int32, device=device)

            # 3. packed_position_ids -> packed_query_position_ids
            if 'packed_position_ids' in kwargs and 'packed_query_position_ids' not in kwargs:
                kwargs['packed_query_position_ids'] = kwargs.pop('packed_position_ids')

            # 4. packed_und_token_indexes -> packed_query_indexes (if missing)
            # In inference, we need to know where the queries map in the global sequence.
            # Usually forward_inference needs explicit 'packed_query_indexes'.
            # If not provided, we might assume 1:1 mapping for simple generation, 
            # but usually the caller should provide this.
    ...
    def forward_train(
        ...
        if self.config.freeze_gen and packed_gen_token_indexes is not None:
            packed_sequence[packed_gen_token_indexes] = packed_sequence[packed_gen_token_indexes].detach()
...
class Qwen2ForCausalLM(Qwen2PreTrainedModel):
    ...
    def forward(self, *args, **kwargs):
        # Same logic: Dispatch to inference path if cache is present
        use_cache = kwargs.pop('use_cache', False) or (kwargs.get('past_key_values') is not None)

        if self.training and not use_cache:
            return self.forward_train(*args, **kwargs)
        else:
            # We map arguments here as well to ensure Qwen2Model receives the correct ones
            if 'packed_sequence' in kwargs and 'packed_query_sequence' not in kwargs:
                kwargs['packed_query_sequence'] = kwargs.pop('packed_sequence')
            
            if 'sample_lens' in kwargs and 'query_lens' not in kwargs:
                sample_lens = kwargs.pop('sample_lens')
                # We can't easily get device here without looking at packed_sequence
                # Let Qwen2Model handle the tensor conversion or do it here if safe
                if isinstance(kwargs.get('packed_query_sequence'), torch.Tensor):
                    device = kwargs['packed_query_sequence'].device
                    kwargs['query_lens'] = torch.tensor(sample_lens, dtype=torch.int32, device=device)
