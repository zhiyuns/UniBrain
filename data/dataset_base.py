...
def build_datasets(local_rank, tokenizer, world_size, num_workers, datasets_metainfo, data_status, stage='train'):
    datasets = []
    is_mandatory = []
    grouped_weights = []
    for grouped_dataset_name, dataset_args in datasets_metainfo.items():
        ...
        for item in dataset_names:
            ...
            if 'split_file' in meta_info.keys():
                if 'split_info' not in dataset_args.keys():
                    dataset_args['split_info'] = {}
                with open(meta_info['split_file'], 'r') as f:
                    split_info = json.load(f)
                dataset_args['split_info'].update(split_info)

        if stage != 'train':
            dataset_args['stage'] = stage
...
class PackedDataset(torch.utils.data.IterableDataset):
    def __init__(
        ...
        grouped_datasets, is_mandatory, grouped_weights = build_datasets(
            self.local_rank, self.tokenizer, self.world_size, self.num_workers,
    ...
    def pack_sequence(self, sample, sequence_status):
        ...
        for item in sequence_plan:
            ...
            if item['type'] == 'text':
                ...
                sequence_status['vit_image_tensors'].append(image_tensor)
