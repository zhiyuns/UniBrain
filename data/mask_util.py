...
def mask2bbox(mask):
    ...
    Args:
        mask (numpy.ndarray): The input mask with shape (h, w, 1) or (h, w).

    Returns:
        tuple[int]: The bounding box in the form of (top, left, height, width).
    """
    ...
    top = np.min(ys)
    left = np.min(xs)
    height = np.max(ys) - top + 1
    width = np.max(xs) - left + 1
