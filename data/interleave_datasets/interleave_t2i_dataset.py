...
class ParquetStandardIterableDataset(DistributedIterableDataset):
    ...
    def __init__(
        ...
        local_rank=0, world_size=1, num_workers=8, data_status=None, stage='train', split_info=None
    ):
        """
        ...
        self.stage = stage
        self.data_paths = self.get_data_paths(data_dir_list, num_used_data, parquet_info, split_info)
        self.set_epoch()

    def get_data_paths(self, data_dir_list, num_used_data, parquet_info, split_info):
        row_groups = []
            ...
            for data_path in data_paths:
                if data_path in parquet_info.keys():
                    ...
                    for rg_idx in range(num_row_groups):
                        if split_info is not None and parquet_info[data_path]['subject_list'][rg_idx] not in split_info[self.stage]:
                            continue
                        else:
