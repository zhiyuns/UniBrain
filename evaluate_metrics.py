...
def process_eval_row(row, task="generation", modalities_in=None, modalities_out=None, mode='2d', replaced_by=None):
    ...
    if mode == '2d':
        ...
        exist_3d = exist_3d > 0
        selected_idxs = np.arange(shape[0]) # np.where(np.any(exist_3d, axis=(1, 2)))[0] if np.any(exist_3d) else np.arange(shape[0])
...
def run_generation_inference(model, vae_model, tokenizer, vae_transform, vit_transform, new_token_ids, task="generation", modalities_in=["t1n", "t2w"], modalities_out=["flair"], save_path=None, row_groups=None, mode='2d', device='cuda', slice_stride=1, sample_time=1):
    inferencer = InterleaveInferencer(model, vae_model, tokenizer, vae_transform, vit_transform, new_token_ids, device=device)
    ...
    for global_row_group_idx, (parquet_file_path, row_group_id) in enumerate(row_groups):
        ...
        for row_idx, row in df.iterrows():
            ...
            for slice_idx, (input_list, all_gt_img) in enumerate(zip(input_lists, all_gt_imgs)):
                if slice_idx % slice_stride != 0: continue
                ...
                for _ in range(sample_time):
                    ...
                    generated_list.append(output_list[0])
            ...
            if mode == '3d':
                p, s = calculate_metrics(generated_imgs[::slice_stride], gt_img_3d[::slice_stride])
            ...
            if save_path is not None:
                ...
                if mode == '2d':
                    ...
                    generated_sitk = sitk.GetImageFromArray(generated_imgs[::slice_stride])
                    gt_sitk = sitk.GetImageFromArray(gt_img_3d[::slice_stride])
...
def run_unified_inference(model, vae_model, tokenizer, vae_transform, vit_transform, new_token_ids, task="unified", modalities_in=["t1n", "t2w"], modalities_out=["flair","t1c"], save_path=None, row_groups=None, sample_time=1, device='cuda'):
    ...
    for global_row_group_idx, (parquet_file_path, row_group_id) in enumerate(row_groups):
        ...
        for row_idx, row in df.iterrows():
            ...
            impression_instruction = 'Assess the given data and present a comprehensive impression of the observed anatomical structures.'
            gen_context_impression = inferencer.update_context_text(impression_instruction, gen_context)
            pred_impression = inferencer.gen_text(gen_context_impression, do_sample=do_sample, temperature=text_temperature, max_length=max_think_token_n)
            print(f"Sample {subject_name} Impression: {pred_impression}")
            with open(f"{save_path}/eval_metrics.txt", 'a') as f:
                f.write(f"Sample {subject_name} Impression: {pred_impression}\n")
            
            final_instruction = 'Based on the provided information, what is the most likely clinical diagnosis? Answer with Glioblastoma, Meningioma, or Metastatic tumor.'
            gen_context_final = inferencer.update_context_text(final_instruction, gen_context_impression)
            pred_label = inferencer.gen_text(gen_context_final, do_sample=do_sample, temperature=text_temperature, max_length=max_think_token_n)
            is_correct = gt_label.lower() in pred_label.lower()  # (pred_label == gt_label)
            if is_correct:
                correct_count += 1
            total_count += 1
...
def main(args):
    ...
    if args.task == 'generation':
        ...
        psnr_list, ssim_list = run_generation_inference(
            ...
            task=args.task, modalities_in=args.modalities_in, modalities_out=args.modalities_out, save_path=save_path, row_groups=row_groups, mode=args.mode, slice_stride=args.slice_stride, sample_time=args.sample_time
        )
        ...
        modality_out_str = '_'.join(args.modalities_out)
        save_path = f"results/eval_samples/{args.task}_{modality_in_str}__{modality_out_str}"
        correct_count, total_count = run_unified_inference(
            model, vae_model, tokenizer, vae_transform, vit_transform, new_token_ids,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--slice_stride", type=int, default=1, help="Stride for selecting slices in 3D evaluation (only for generation task)")
